...
class AskDataGraph:
    ...
    async def run(self, question: str) -> AgentResult:
        ...
        state: AskDataGraphState = {
            ...
            "recorder": self._trace_recorder_factory(question, trace_id),
    ...
    async def evaluate_intent(self, state: AskDataGraphState) -> dict[str, Any]:
        ...
        decision = evaluate_intent_policy(state["question"])
    ...
    async def generate_sql(self, state: AskDataGraphState) -> dict[str, Any]:
        ...
        llm = state["llm"]
        try:
            generated_sql = await llm.generate_sql(state["question"], SCHEMA_CONTEXT)
            ...
            return {
                ...
                "policy_reason": str(exc),
                "skip_steps": ["sql_validation", "query_execution"],
        ...
        return {"generated_sql": generated_sql, "sql": generated_sql}
    ...
    async def execute_query(self, state: AskDataGraphState) -> dict[str, Any]:
        ...
        try:
            tool_result = state["query_tool"].run(decision)
    ...
    async def render_answer(self, state: AskDataGraphState) -> dict[str, Any]:
        ...
        answer = render_rows_as_answer(state["question"], tool_result.rows)
        state["recorder"].record_step(
            "answer_rendering",
            "ok",
...
def build_ask_data_graph(nodes: AskDataGraph):
    ...
    graph.add_conditional_edges(
        "intent_policy",
        _route_after_intent,
        {"allowed": "provider_resolution", "terminal": "record_skipped_downstream"},
    )
    graph.add_conditional_edges(
        "provider_resolution",
...
def _route_after_generation(state: AskDataGraphState) -> str:
    return "ready" if "generated_sql" in state else "terminal"


def _route_after_validation(state: AskDataGraphState) -> str:
    decision = state["sql_decision"]
    return "ready" if decision.status == "allowed" else "terminal"
